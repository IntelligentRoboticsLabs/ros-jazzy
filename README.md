# IRL ROS jazzy buildfarm (Pixi + Vinca + Rattler)

This repository is a fork of the upstream RoboStack ROS jazzy packaging repo:

- https://github.com/RoboStack/ros-jazzy

In this fork, we use the same toolchain (Pixi + Vinca + rattler-build) to build and publish **additional ROS 2 jazzy packages** produced by the **Intelligent Robotics Lab (IRL)**:

- https://intelligentroboticslab.gsyc.urjc.es/

The resulting binaries are published to prefix.dev (the `irl-jazzy` channel) and can also be consumed locally via a file-based channel.
...
- Prefix.dev channels:
    - [`irl-jazzy`](https://prefix.dev/channels/irl-jazzy)
    - [`robostack-jazzy`](https://prefix.dev/channels/robostack-jazzy)
